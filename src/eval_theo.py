...
def evaluate_theoretical_causality(var, parameters, time_list):
    """
    ...
    time_list: list([start, stop, num_timesteps])
        Time config. 
    ...
    start, stop, num_timepoints = time_list
...
def plot_theoretical_causality(mu, var, parameters, time_list):

    # 1. Calculate the information flow values, check shape
    t32_values, t21_values, s11_values = evaluate_theoretical_causality(var, parameters, time_list)
    ...
    t_array = np.linspace(time_list[0], time_list[1], time_list[2])
...
plot_theoretical_causality([1, 0.6, -0.2216, 0], [5, 5, 5, 5], [10, 0.4, 1, 3.1], [0, 20, 500])